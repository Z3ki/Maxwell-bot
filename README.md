...
- Running `shell` needs a working Docker daemon the bot user can reach. On Linux, compose bind-mounts the host `docker` CLI plus `docker.sock` — there is no second client inside the image. Without a daemon the tool reports the failure rather than silently falling back to the host. `python3 doctor.py` tells you which side you are on.
