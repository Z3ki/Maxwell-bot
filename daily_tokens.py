"""Persistent, concurrency-safe daily token accounting for live user requests."""
...
class DailyTokens:
    def __init__(self, path: str | Path):
        ...
        with self._db() as db:
            db.execute("CREATE TABLE IF NOT EXISTS usage (day TEXT NOT NULL, user_id TEXT NOT NULL, spent INTEGER NOT NULL DEFAULT 0, reserved INTEGER NOT NULL DEFAULT 0, PRIMARY KEY(day, user_id))")
    ...
    def status(self, user_id: str, default_limit: int) -> dict:
        day = self.today()
        with self._db() as db:
            row = db.execute("SELECT spent, reserved FROM usage WHERE day=? AND user_id=?", (day, str(user_id))).fetchone() or (0, 0)
        ...
        return {"day": day, "user_id": str(user_id), "spent": row[0], "reserved": row[1], "limit": limit, "override": override[0], "exempt": bool(override[1])}
    ...
    def reserve(self, user_id: str, default_limit: int, prompt_estimate: int, max_output: int) -> tuple[str | None, int]:
        ...
        with self._db() as db:
            ...
            if output < 1:
                raise DailyTokenLimitExceeded(f"Daily token limit reached ({row[0]:,}/{limit:,} used). Resets at 00:00 UTC.")
    ...
    def settle(self, reservation_id: str | None, actual: int) -> None:
        ...
        with self._db() as db:
            ...
            if row is None:
                return
            db.execute("UPDATE usage SET spent=spent+?, reserved=MAX(0,reserved-?) WHERE day=? AND user_id=?", (max(0, int(actual)), row[2], row[0], row[1]))
