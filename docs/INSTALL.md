...
To gate who can add the official bot, serve `/install` on the dashboard origin and set Discord Developer Portal → Installation → Install Link to **Custom URL** `https://<dashboard-host>/install`. Visitors must log in with Discord and be in `admins.json` / `MAXWELL_OWNER_IDS` before Maxwell hands them Discord's Add App URL. After login, admins can **Add to a server** (guild bot) or **Add to my apps** (user-install). User-install still requires the same admin list. Commands: `/maxwell`, right-click a message → **Ask Maxwell** / **Summarize**, right-click a user → **Ask Maxwell**. Message/user menus include the clicked item. If Maxwell is also in that server, `/maxwell` snapshots recent channel messages. Anyone else who invokes is refused. Discord rejects Add to my apps with "installation type not supported" until the app has a user-install context; Maxwell enables that on startup via `PATCH /applications/@me` if it is missing.
