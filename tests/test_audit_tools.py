...
def test_bot_http_multipart_retry_rebuilds_without_duplicate_files(monkeypatch):
    class Response:
        ...
        async def json(self, **kwargs):
            return {"retry_after": 0} if self.status == 429 else {"id": "message"}
    ...
    class Session:
        ...
        def request(self, *args, **kwargs):
            body = kwargs["data"]
            names = [field[0]["name"] for field in body._fields]
            assert names == ["payload_json", "files[0]"]
            assert all(body is not previous for previous in self.bodies)
            self.bodies.append(body)
            body()  # aiohttp consumes FormData to make its request payload
            return Response(429 if len(self.bodies) == 1 else 200)

    session = Session()
    http = SimpleNamespace(
        _bot_account=True, token="Bot dummy", _maxwell_aiohttp=session
    )
    file = discord.File(io.BytesIO(b"payload"), filename="test.txt")
    form = [
        {"name": "payload_json", "data": "{}"},
        {"name": "files[0]", "data": file.fp, "filename": file.filename},
    ]
    monkeypatch.setattr(discord_account.asyncio, "sleep", AsyncMock())
    result = run(
        discord_account._aiohttp_bot_request(
            http,
            SimpleNamespace(method="POST", url="https://discord.invalid"),
            files=[file],
            form=form,
        )
    )
...
def test_rest_gate_spaces_concurrent_requests_and_rechecks_cooldown(monkeypatch):
    async def scenario():
        gate = discord_account.UserRestGate(min_interval=1, max_inflight=2)
        clock = [100.0]
        admissions = []
        sleeps = []
        monkeypatch.setattr(discord_account.time, "monotonic", lambda: clock[0])
        ...
        monkeypatch.setattr(discord_account.asyncio, "sleep", sleep)
        async with gate.slot():
            admissions.append(clock[0])
            async with gate.slot():
                admissions.append(clock[0])
        assert admissions == [100.0, 103.0]
        assert sleeps == [1.0, 2.0]

    run(scenario())


def optional_onboarding():
    return {
        "enabled": True,
        "prompts": [
            {
                "id": "p",
                "required": False,
                "options": [{"id": "o", "title": "Notifications"}],
            }
        ],
    }


def test_onboarding_respects_explicit_empty_optional_picks():
    from guild_onboarding import run_onboarding

    async def scenario():
        request = AsyncMock(side_effect=[optional_onboarding(), {}])
        result = await run_onboarding(
            request, 1, "test", ask_llm=AsyncMock(return_value='{"picks": []}')
        )
        assert result["ok"]
        assert result["picked_by"] == "model"
        assert result["choice"] == {}
        assert request.await_args.args[2]["onboarding_responses"] == []

    run(scenario())


def test_onboarding_parses_list_surrounded_by_prose():
    from guild_onboarding import normalize_prompts, parse_choice_json

    text = 'My choices: [{"prompt_id": "p", "option_ids": ["o"]}] thanks'
    assert parse_choice_json(text, normalize_prompts(optional_onboarding())) == {
        "p": ["o"]
    }


@pytest.mark.parametrize(
    "payload", [[1], {"prompts": 3}, {"prompts": [{"id": "p", "options": 7}]}]
)
def test_onboarding_bad_api_shape_returns_failure(payload):
    from guild_onboarding import run_onboarding

    result = run(run_onboarding(AsyncMock(return_value=payload), 1, "test"))
